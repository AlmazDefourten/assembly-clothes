...
function formFurnitureFun($attrs){
	...
	$vendors = $wpdb->get_results("SELECT ID, display_name FROM wp_users WHERE ID IN (SELECT DISTINCT wendorId FROM wp_furns_price WHERE term_id IN (SELECT term_id FROM wp_terms WHERE term_id IN (SELECT DISTINCT t.term_id FROM wp_posts AS p INNER JOIN wp_term_relationships AS tr ON p.id = tr.object_id INNER JOIN wp_term_taxonomy AS tt ON tt.term_taxonomy_id = tr.term_taxonomy_id INNER JOIN wp_terms AS t ON t.term_id = tt.term_id WHERE p.ID = $productId AND tt.taxonomy = 'pa_фурнитура' AND p.post_type = 'product' AND tt.taxonomy LIKE 'pa_%')))") ;
	?>
...
<?
	foreach($dop as $res){
		...
		<td><input name="<?$res->term_id?>" required="" type="number" /></td>
		</tr>
...
function add_estimate(){
	...
	$query=[
		...
		if(trim(iconv_substr(strip_tags($_POST['getMethod']), 0, 100))=="insert"){
			...
			$wpdb->update($wpdb->prefix.'cost_estimate',$query,['wendorId'=>$wendorId,'cardProductId'=>$cardProductId]);
		}
